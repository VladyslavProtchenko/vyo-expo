...
export const useSaveProfile = () => {
  ...
  const saveProfileData = async (): Promise<SaveProfileResult> => {
    ...
    try {
      // ДЕМО: Закомментировано сохранение в Supabase
      ...
      // ДЕМО: Обновляем только локальный стейт
      profileData.setValue(true, 'finished');

      console.log('✅ Profile data saved successfully (demo mode)');
      return { success: true };
...
export const useCheckOnboarding = (): ProfileStatus => {
  ...
  useEffect(() => {
    // ДЕМО: Закомментирована проверка onboarding в Supabase
    ...
    // ДЕМО: Используем локальный стейт
    const profileData = useProfileStore.getState();
    setOnboardingCompleted(profileData.finished || false);
    setLoading(false);
  }, []);
